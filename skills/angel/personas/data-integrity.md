...
- **Silent-success metrics**: flag log/metric lines that measure the mechanic ("HTTP 200", "rows inserted", "API called") instead of the effect ("linked N records to parents", "resolved N contacts to accounts"). This instrumentation can let absent-write bugs hide across repeated runs: a sync logging "HTTP 200, rows synced" while inserting rows with a NULL FK is the canonical pattern this persona exists to catch — flag the log line, not just the insert.
...
**Flag this** — a sync adapter that inserts rows with `parent_id = NULL` because the parent lookup isn't threaded into the insert. Consumer code joins `items JOIN parents ON items.parent_id = parents.id` and silently returns empty groupings. Attack vector (effectively): the entire feature is broken, but the sync logs "HTTP 200, rows synced." Fix: add a `find_or_create_parent_by_external_id` call, thread the resulting id into the insert, add a NOT NULL constraint + migration.

**Flag this** — an onboarding `storeCredentials(tenantId, token)` that silently returns early if a global `API_TOKEN=` env var exists, clobbering the per-tenant token. Fix: per-tenant keying (`SESSION_TOKEN_<tenantId>`), upsert not skip, boot-time loud error when a tenant is configured but has no token.
